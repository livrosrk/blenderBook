...
- Based on [this tutorial](https://www.youtube.com/watch?v=AJJNX243k9E)

### QGIS
- Export the dem and the overlay with the same settings under: 

`Project -> Import/Export -> And get the extent from the dem layer`

- Here set the Pixels first (by using the extent from the layer) and then set the dpi

### Blender

- in Blender add a `plane`, go to edit mode with `tab`, right click on the plane and click **subdivide**. Then click many times (maybe 5) `shift + R` to repeat the action and subdivide it many times. 

- Add a **modifier**. In this case a `displacement modifier` and call it DEM. 

- By alternating the `strength` option you can kind of change the *z-level*
![](images/imageTopModifier.png)

- Add a **texture** either by clicking in the above menu on the two lines 

- Go to `open` and select the image


#### Add the overlay map (e.g. OSM)

- You need to add a **material**
